...
@interface BussizeDetailViewController ()
{
//当前
...
@implementation BussizeDetailViewController
...
- (void)viewDidLoad
{
    ...
    segmented.tintColor= [UIColor colorWithRed:0.23 green:0.50 blue:0.82 alpha:0.90];
    ...
    tableView = [[UITableView alloc] initWithFrame:CGRectMake(0 , 0, ScreenWidth,  ScreenHeight - 114)];
    ...
    dispatch_async(dispatch_get_global_queue( DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
        ...
        [self requestRecordList:str withEndDate:str withStart:start withSize:limit tag:kBusinessTagGetJRtransRecordList];

        [self requestRecordList:notStarDate withEndDate:notEddDate withStart:startPast withSize:limitPast tag:kBusinessTagGetJRtransRecordListPast];
  ...
  //kBusinessTagGetJRtransRecordListAgain
...
- (void)callPhone:(UIGestureRecognizer *)sender
{
    ...
    timeTooBar.hidden = YES;
    timePicker.hidden = YES;
    datePicker.date = [self dateFromString:notStarDate];
    ...
    if ([earlyDate isEqualToDate:endDate] && ![earlyDate isEqualToDate:beginDate]) {
        ...
        dispatch_async(dispatch_get_global_queue( DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
            ...
            startPast = @"0";
             ...
             [self requestRecordList:notStarDate withEndDate:notEddDate withStart:startPast withSize:limitPast tag:kBusinessTagGetJRtransRecordListPastAgain];
            dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)slimeRefreshStartRefresh:(SRRefreshView *)refreshView
{
    if (refreshView == _slimeView) {
        ...
        start = @"0";
        NSString *str =[self dateToStringDate:[NSDate date]];
        [self requestRecordList:str withEndDate:str withStart:start withSize:limit tag:kBusinessTagGetJRtransRecordListAgain];
    ...
    if (tbleView == tableView) {
        ...
        if ([indexPath row] == [dataList count]) {
            ...
            if (cell == nil) {
                ...
                dateLab.text = [NSString stringWithFormat:@"%@  %@",strDate,[[dataList objectAtIndex:[indexPath row]] objectForKey:@"FID_SQSJ"]];
        ...
        if ([dataListPast count] == 0) {
            ...
            if ([indexPath row] == [dataListPast count]) {
                ...
                if (cell == nil) {
                    ...
                    [strDate insertString:@"-" atIndex:4];
                    [strDate insertString:@"-" atIndex:(strDate.length - 2)];
                    dateLab.text = [NSString stringWithFormat:@"%@  %@",strDate,[[dataListPast objectAtIndex:[indexPath row]] objectForKey:@"FID_SQSJ"]];
                    dateLab.font = [UIFont systemFontOfSize:14];
                    dateLab.textColor = [ColorUtil colorWithHexString:@"999999"];
                    [backView addSubview:dateLab];
...
-(void)endPost:(NSString *)result business:(kBusinessTag)tag{
   ...
   if (tag==kBusinessTagGetJRtransRecordList) {
    ...
    }else if (tag == kBusinessTagGetJRtransRecordListAgain){
        NSMutableArray *dataArray = [jsonDic objectForKey:@"object"];
        [_slimeView endRefresh];
        if ([[jsonDic objectForKey:@"success"] boolValue] == 0) {
            //数据异常处理
    ...
    } else if (tag==kBusinessTagGetJRtransRecordListPast) {
        NSMutableArray *dataArray = [jsonDic objectForKey:@"object"];
        if ([[jsonDic objectForKey:@"success"] boolValue] == 0) {
            //数据异常处理
            if ([jsonDic objectForKey:@"object"] == [NSNull null]) {
                dataListPast = [NSMutableArray array];
            } else {
            if ([[jsonDic objectForKey:@"object"] isEqualToString:@"loginTimeout"] == YES) {
    ...
    }else if (tag == kBusinessTagGetJRtransRecordListPastAgain){
        NSMutableArray *dataArray = [jsonDic objectForKey:@"object"];
        [_slimeViewPast endRefresh];
        if ([[jsonDic objectForKey:@"success"] boolValue] == 0) {
            //数据异常处理
